...
def blur_regions(frame, regions, blur_factor):
    ...
    for region in regions_array:
        ...
        try:
            ...
            print(f"WARN: Blur failed. ROI:{roi.shape} K:({kernel_size},{kernel_size}) Err:{e}")
            continue
    return processed_frame
...
def process_uploaded_image(input_path, output_path, blur_factor, manual_regions):
    ...
    try:
        ...
        if img is None:
            raise IOError(f"Could not read input image file: {input_path}")
        if manual_regions:
            print(f" INFO: Using {len(manual_regions)} manually drawn region(s).")
            regions_to_blur = manual_regions
            method_used = "Manual"
        elif haar_cascade:
            print(f" INFO: No manual regions, using automatic Haar detection.")
            regions_to_blur = detect_faces_haar(img)
            method_used = "Automatic (Haar)"
            print(f" INFO: Auto-detected {len(regions_to_blur)} face regions (Haar).")
        else:
            print(" INFO: No manual regions and Haar cascade not loaded. No blurring.")
            method_used = "None"
        blurred_img = blur_regions(img, regions_to_blur, blur_factor)
        success = cv2.imwrite(output_path, blurred_img)
        if not success:
            raise IOError(f"Could not write processed image to: {output_path}")
...
def process_file():
    ...
    if not (0.05 <= blur_factor <= 1.0):
        print(f"WARN: Blur factor {blur_factor} out of range [0.05, 1.0], clamping not implemented, using original or default.")
        blur_factor = max(0.05, min(1.0, blur_factor))  # Basic clamping example
    ...
    try:
        ...
        print(f"Error creating upload dir: {e}")
        return jsonify({"success": False, "error": "Server directory error."}), 500
...
def uploaded_file(filename):
    ...
    if not safe_path.startswith(safe_dir):
        print(f"WARN: Forbidden access attempt: {filename}")
        return "Forbidden", 403
    try:
        print(f"INFO: Serving file: {filename} from {safe_dir}")
        return send_from_directory(current_app.config['UPLOAD_FOLDER'], filename, as_attachment=False)
    except FileNotFoundError:
        print(f"ERROR: File not found requested: {filename}")
