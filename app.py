# app.py (Combined Auto Haar + Manual Draw - Standard Filenames v6.1)
...
# --- Flask App Setup ---
app = Flask(__name__) # Looks for 'templates' folder in the same directory (BASE_DIR)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER_PATH
app.config['MAX_CONTENT_LENGTH'] = MAX_CONTENT_LENGTH
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'combined_blur_secret_standard_name_v2') # Change this!
...
def check_folder_permissions(folder_path):
    """Checks write permissions for a folder."""
    ...
    except Exception as e: print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"); print(f"ERROR: Cannot write to upload folder: {folder_path}"); print(f"       Reason: {e}"); print(f"       Please check directory permissions!"); print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"); return False
...
def process_uploaded_image(input_path, output_path, blur_factor, manual_regions):
    ...
    method_used="Error" # Initialize
    try:
        img = cv2.imread(input_path)
        if img is None: raise IOError(f"Could not read input image file: {input_path}")
        regions_to_blur = [];
        if manual_regions:
            print(f" INFO: Using {len(manual_regions)} manually drawn region(s)."); regions_to_blur = manual_regions; method_used = "Manual"
        elif haar_cascade:
            print(f" INFO: No manual regions, using automatic Haar detection."); regions_to_blur = detect_faces_haar(img); method_used = "Automatic (Haar)"; print(f" INFO: Auto-detected {len(regions_to_blur)} face regions (Haar).")
...
def process_uploaded_video(input_path, output_path, blur_factor, manual_regions):
    """Loads video, processes frame-by-frame (manual or auto), saves new video."""
    print(f"--- Processing Video: {os.path.basename(input_path)} ---")
    cap = None; out = None; regions_to_use = []; using_manual = False; total_detections_this_video = 0
    method_used = "Error" # Initialize
    if manual_regions:
        ...
        print(f" INFO: No manual regions, using automatic Haar detection per frame."); method_used = "Automatic (Haar)"
...
def index():
    ...
    try:
        return render_template('index.html') # Use standard name
    except Exception as e:
        print(f"CRITICAL ERROR rendering template 'index.html': {e}"); traceback.print_exc()
        return f"Internal Server Error: Could not render template 'index.html'. Check server logs and ensure file exists in '{app.template_folder}'.", 500
...
def process_file():
    ...
    if not regions_were_provided and haar_cascade is None: print("ERROR: No manual regions & Haar cascade not loaded."); return jsonify({"success": False, "error": "Cannot process: No manual regions drawn and face detector is not available."}), 400
